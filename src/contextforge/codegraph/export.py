...
data.nodes.forEach(n=>{{const c=document.createElementNS(NS,'circle');c.setAttribute('cx',n.x);c.setAttribute('cy',n.y);c.setAttribute('r',n.radius);c.setAttribute('fill',palette[Math.abs(Number(n.community)||0)%palette.length]);c.setAttribute('class','node');c.dataset.id=n.id;c.addEventListener('click',()=>selectNode(n.id));nodeLayer.append(c);if((degree.get(n.id)||0)>=data.labelThreshold){{const t=document.createElementNS(NS,'text');t.setAttribute('x',n.x+9);t.setAttribute('y',n.y+3);t.setAttribute('class','label');t.textContent=n.label;labelLayer.append(t)}}}});
...
def _visual_payload(payload: dict[str, Any], *, limit: int = 700) -> dict[str, Any]:
    ...
    for group_index, (community, members) in enumerate(sorted(grouped.items())):
        ...
        for member_index, node in enumerate(sorted(members, key=lambda item: str(item["id"]))):
            ...
            record["radius"] = round(4.0 + 7.0 * math.sqrt(float(node.get("centrality", 0.0))), 2)
